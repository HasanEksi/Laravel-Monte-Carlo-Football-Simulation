...
const generateFixtures = () => {
    swal.fire({
        title: 'Are you sure?',
        text: 'This will generate fixtures for all teams in the system.',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'Yes, generate fixtures!'
    }).then((result) => {
        if (result.isConfirmed) {
            axios.post(route('fixtures.store')).then((response) => {

                location.href = route('fixtures.index');
